...
class CostOfSales6076ControllerSpec extends ControllerSpec:
  ...
  "GET /" should {
    "return 200" in {
      val result = costOfSales6076Controller.show(getRequest)
    ...
    "render back link to CYA if come from CYA" in {
      val result  = costOfSales6076Controller.show(getRequestFromCYA)
    ...
    "return correct backLink when 'from=IES' query param is present" in {
      val result = costOfSales6076Controller.show()(FakeRequest(GET, "/path?from=IES"))
