...
class GrossReceiptsLettingUnitsControllerSpec extends ControllerSpec:
  ...
  "GET /" should {
    "return 200" in {
      val result = grossReceiptsLettingUnitsController.show(getRequest)
    ...
    "render back link to CYA if come from CYA" in {
      val result  = grossReceiptsLettingUnitsController.show(getRequestFromCYA)
      val content = contentAsString(result)
