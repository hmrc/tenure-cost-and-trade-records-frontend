...
class VariableOperatingExpensesControllerSpec extends TestBaseSpec:
  ...
  val variableOperatingExpensesController: VariableOperatingExpensesController =
    VariableOperatingExpensesController(
  ...
  "GET /" should {
    ...
    "return correct backLink when 'from=IES' query param is present" in {
      val result = variableOperatingExpensesController.show()(FakeRequest(GET, "/path?from=IES"))
  ...
  "SUBMIT /" should {
    "throw a BAD_REQUEST if an empty form is submitted" in {
      val res = variableOperatingExpensesController.submit(
