...
class PercentageFromFuelCardsControllerSpec extends ControllerSpec:
  ...
  "Percentage from fuel cards controller" should {
    "return 200" in {
      val result = percentageFromFuelCards().show(getRequest)
    ...
    "render back link to CYA if come from CYA" in {
      val result  = percentageFromFuelCards().show(getRequestFromCYA)
  ...
  "SUBMIT /" should {
    "throw a BAD_REQUEST if an empty form is submitted" in {
      val res = percentageFromFuelCards().submit(getRequest.withFormUrlEncodedBody(Seq.empty*))
