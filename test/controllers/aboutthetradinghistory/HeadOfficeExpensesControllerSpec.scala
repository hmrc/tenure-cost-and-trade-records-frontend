...
class HeadOfficeExpensesControllerSpec extends ControllerSpec:
  ...
  "GET /" should {
    "return 200" in {
      val result = headOfficeExpensesController.show(getRequest)
    ...
    "render back link to CYA if come from CYA" in {
      val result  = headOfficeExpensesController.show(getRequestFromCYA)
      val content = contentAsString(result)
