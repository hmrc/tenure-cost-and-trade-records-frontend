...
class OtherLettingControllerSpec extends ControllerSpec with JsoupHelpers with Inside:
  ...
  "the OtherLetting controller" when {
    "handling GET requests" should {
      ...
      "reply 200 with a pre-filled HTML form if given a known index" in new ControllerFixture {
        val result: Future[Result] = controller.show(index = Some(3))(getRequest)
        val page: Document         = contentAsJsoup(result)
      ...
      "render back link to CYA if come from CYA" in new ControllerFixture {
        val result: Future[Result] = controller.show(Some(3))(getRequestFromCYA)
        val page: Document         = contentAsJsoup(result)
    ...
    "handling POST requests" should {
      ...
      "save new record and reply 303 and redirect to address lookup page" in new ControllerFixture(havingNoLettings = true) {
        ...
        inside(session.getValue.aboutFranchisesOrLettings.get.lettings.get.apply(0)) { case record: OtherLetting =>
          record.lettingType.get shouldBe lettingType
          record.tenantName.get  shouldBe tenantName
        }
      }
      ...
      "update existing record and reply 303 and redirect to address lookup page" in new ControllerFixture {
        ...
        val session: ArgumentCaptor[Session] = captor[Session]
        verify(repository, once).saveOrUpdate(session.capture())(using any)
        inside(session.getValue.aboutFranchisesOrLettings.get.lettings.get.apply(3)) { case record: OtherLetting =>
          record.lettingType.get shouldBe lettingType
          record.tenantName.get  shouldBe tenantName
        }
      }
    ...
    "retrieving the confirmed address" should {
      "save record and reply 303 redirect to the next page" in new ControllerFixture {
        val result: Future[Result] = controller.addressLookupCallback(idx = 3, "confirmedAddress")(getRequest)
        status(result)               shouldBe SEE_OTHER
        redirectLocation(result).get shouldBe routes.RentDetailsController.show(idx = 3).url
