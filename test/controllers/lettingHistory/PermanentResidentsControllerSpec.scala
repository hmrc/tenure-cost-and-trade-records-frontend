...
class PermanentResidentsControllerSpec extends LettingHistoryControllerSpec:
  ...
  "the PermanentResidents controller" when {
    "the user session is fresh" should {
      ...
      "be handling invalid POST by replying 400 with error message" in new FreshSessionFixture {
      ...
      "be handling POST answer='yes' by replying 303 redirect to the 'Residents Details' page" in new FreshSessionFixture {
        val result = controller.submit(
          fakePostRequest.withFormUrlEncodedBody(
            "answer" -> "yes"
          )
    ...
    "the user session is stale" should {
      "regardless of the given number of residents"          should {
        ...
        "be handling POST answer='yes' by replying 303 redirect to the 'Resident Detail' page" in new StaleSessionFixture(
          oneResident
        ) {
        ...
        "be handling POST answer='no' by replying 303 redirect to the 'Commercial Lettings' page" in new StaleSessionFixture(
          oneResident
        ) {
      ...
      "and the maximum number of residents has been reached" should {
        "be handling POST answer='yes' by replying 303 redirect to the 'Resident List' page" in new StaleSessionFixture(
          fiveResidents
        ) {
          val result = controller.submit(fakePostRequest.withFormUrlEncodedBody("answer" -> "yes"))
          status(result)                    shouldBe SEE_OTHER
  ...
  trait StaleSessionFixture(permanentResidents: List[ResidentDetail] = Nil)
