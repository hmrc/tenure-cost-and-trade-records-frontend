...
class BatteriesCapacityControllerSpec extends ControllerSpec:
  ...
  "GET /" should {
    "GET / return 200 about you in the session" in {
      val result = batteriesCapacityController().show(getRequest)
    ...
    "GET / return 200 plant and technology in the session" in {
      val result = batteriesCapacityControllerNone().show(getRequest)
      status(result)      shouldBe OK
      contentType(result) shouldBe Some("text/html")
      charset(result)     shouldBe Some("utf-8")
    }
