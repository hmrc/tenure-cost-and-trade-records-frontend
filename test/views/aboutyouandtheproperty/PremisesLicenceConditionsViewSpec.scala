...
class PremisesLicenceConditionsViewSpec extends QuestionViewBehaviours[AnswersYesNo]:
  ...
  private def createView: () => Html =
    () => premisesLicensableView(form, backLink, Summary("99996010001"))(using getRequest, messages)

  private def createViewUsingForm: Form[AnswersYesNo] => Html =
    form => premisesLicensableView(form, backLink, Summary("99996010001"))(using getRequest, messages)

  "Property licence conditions view" should {

    behave like normalPage(createView, messageKeyPrefix)

    behave like pageWithBackLink(createView, "Licensable Activities", backLink)
