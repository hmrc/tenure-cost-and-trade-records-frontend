...
class ContactDetailsQuestionViewSpec extends QuestionViewBehaviours[AnswersYesNo]:
  ...
  private def createView: () => Html =
    () => contactDetailsQuestionView(form, Summary("99996010001"), false)(using getRequest, messages)

  private def createViewUsingForm: Form[AnswersYesNo] => Html =
    form => contactDetailsQuestionView(form, Summary("99996010001"), false)(using getRequest, messages)

  "Contact details question view" should {

    behave like normalPage(createView, messageKeyPrefix)

    behave like pageWithBackLink(createView, "About You", controllers.aboutyouandtheproperty.routes.AboutYouController.show().url)
