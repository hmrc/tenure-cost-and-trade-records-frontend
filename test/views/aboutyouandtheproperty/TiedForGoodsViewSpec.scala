...
class TiedForGoodsViewSpec extends QuestionViewBehaviours[AnswersYesNo]:
  ...
  def createView: () => Html =
    () => tiedForGoodsView(form, backLink, Summary("99996010001"))(using getRequest, messages)

  def createViewUsingForm: Form[AnswersYesNo] => Html =
    form => tiedForGoodsView(form, backLink, Summary("99996010001"))(using getRequest, messages)
