...
class TradingActivityViewSpec extends QuestionViewBehaviours[TradingActivity]:
  ...
  private def createView: () => Html =
    () => tradingActivityView(form, Summary("99996010001"))(using getRequest, messages)

  private def createViewUsingForm: Form[TradingActivity] => Html =
    form => tradingActivityView(form, Summary("99996010001"))(using getRequest, messages)

  "Trading activity view" should {

    behave like normalPage(createView, messageKeyPrefix)

    behave like pageWithBackLink(createView, "Charity question", controllers.aboutyouandtheproperty.routes.CharityQuestionController.show().url)
