...
class WebsiteForPropertyViewSpec extends QuestionViewBehaviours[WebsiteForPropertyDetails]:
  ...
  private def createView: () => Html =
    () => websiteForPropertyView(form, Summary("99996010001"), backLink)(using getRequest, messages)

  private def createViewUsingForm: Form[WebsiteForPropertyDetails] => Html =
    form => websiteForPropertyView(form, Summary("99996010001"), backLink)(using getRequest, messages)

  "Property website view" should {

    behave like normalPage(createView, messageKeyPrefix)

    behave like pageWithBackLink(createView, "About the property", backLink)
