...
class TiedForGoodsDetailsViewSpec extends QuestionViewBehaviours[TiedForGoodsInformationDetails]:
  ...
  def createView: () => Html = () => tiedForGoodsDetailsView(form, Summary("99996010001"))(using getRequest, messages)

  def createViewUsingForm: Form[TiedForGoodsInformationDetails] => Html =
    form => tiedForGoodsDetailsView(form, Summary("99996010001"))(using getRequest, messages)

  "Tied for goods details view" should {

    behave like normalPage(createView, messageKeyPrefix)

    behave like pageWithBackLink(createView, "Tied for goods", controllers.aboutyouandtheproperty.routes.TiedForGoodsController.show().url)
