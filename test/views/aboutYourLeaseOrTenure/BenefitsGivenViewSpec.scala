...
class BenefitsGivenViewSpec extends QuestionViewBehaviours[AnswersYesNo]:
  ...
  private def createView = () => benefitsGivenView(form, Summary("99996010001"))(using getRequest, messages)

  private def createViewUsingForm = (form: Form[AnswersYesNo]) =>
    benefitsGivenView(form, Summary("99996010001"))(using getRequest, messages)

  "Benefits given view" should {

    behave like normalPage(createView, messageKeyPrefix)

    behave like pageWithBackLink(createView, "Lease Surrendered Early", controllers.aboutYourLeaseOrTenure.routes.LeaseSurrenderedEarlyController.show().url)
