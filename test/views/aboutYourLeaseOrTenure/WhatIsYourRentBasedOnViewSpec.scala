...
class WhatIsYourRentBasedOnViewSpec extends QuestionViewBehaviours[WhatIsYourCurrentRentBasedOnDetails]:
  ...
  override val form: Form[WhatIsYourCurrentRentBasedOnDetails] = WhatIsYourCurrentRentBasedOnForm.whatIsYourCurrentRentBasedOnForm

  private def createView = () => whatIsYourRentBasedOnView(form, Summary("99996010001"))(using getRequest, messages)

  private def createViewUsingForm = (form: Form[WhatIsYourCurrentRentBasedOnDetails]) =>
    whatIsYourRentBasedOnView(form, Summary("99996010001"))(using getRequest, messages)
  ...
  "Current rent based on details view" should {
    ...
    behave like pageWithBackLink(createView, "Rent Open Market Value", controllers.aboutYourLeaseOrTenure.routes.RentOpenMarketValueController.show().url)
