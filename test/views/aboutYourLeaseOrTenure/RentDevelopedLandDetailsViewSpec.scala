...
class RentDevelopedLandDetailsViewSpec extends QuestionViewBehaviours[String]:
  ...
  private def createView = () => rentDevelopedLandDetailsView(form, Summary("99996010001"))(using getRequest, messages)

  private def createViewUsingForm = (form: Form[String]) =>
    rentDevelopedLandDetailsView(form, Summary("99996010001"))(using getRequest, messages)

  "Rented Equipment Details view" should {

    behave like normalPage(createView, messageKeyPrefix)

    behave like pageWithBackLink(createView, "Rent Developed Land", controllers.aboutYourLeaseOrTenure.routes.RentDevelopedLandController.show().url)
