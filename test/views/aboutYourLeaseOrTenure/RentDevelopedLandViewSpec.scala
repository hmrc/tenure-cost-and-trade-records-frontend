...
class RentDevelopedLandViewSpec extends QuestionViewBehaviours[AnswersYesNo]:
  ...
  private def createView = () => rentDevelopedLandView(form, Summary("99996010001"))(using getRequest, messages)

  private def createViewUsingForm = (form: Form[AnswersYesNo]) =>
    rentDevelopedLandView(form, Summary("99996010001"))(using getRequest, messages)

  "Rent Developed Land view" should {

    behave like normalPage(createView, messageKeyPrefix)

    behave like pageWithBackLink(createView, "Does The Rent Payable", controllers.aboutYourLeaseOrTenure.routes.DoesTheRentPayableController.show().url)
