...
class PropertyUseLeasebackArrangementViewSpec extends QuestionViewBehaviours[AnswersYesNo]:
  ...
  private def createView = () =>
    propertyUseLeasebackArrangementView(form, backLink, "{0}", Summary("99996010001"))(using getRequest, messages)
  ...
  "Property use leaseback arrangement view" should {
    ...
    behave like pageWithBackLink(createView, "Lease Or Agreement Years", backLink)
    ...
    "contain radio buttons for property use leaseback arrangement with the value yes" in {
      val doc = asDocument(createViewUsingForm(form))
    ...
    "contain radio buttons for property use leaseback arrangement with the value no" in {
      val doc = asDocument(createViewUsingForm(form))
      assertContainsRadioButton(
        doc,
