...
class FinancialYearEndViewSpec extends QuestionViewBehaviours[(DayMonthsDuration, Boolean)]:
  ...
  "financialYearEnd view" should {
    ...
    behave like pageWithBackLink(createView, "Date First Occupy", controllers.aboutthetradinghistory.routes.WhenDidYouFirstOccupyController.show().url)
    ...
    "contain date format hint for financialYear-hint" in {
      ...
      financialYearHint shouldBe messages("hint.day.month.example")
    }
