...
class SensitiveRequestReferenceNumberAddressSpec
    extends AnyFlatSpec
    with Matchers
    with OptionValues
  ...
  it should "encrypt and decrypt sensitive fields correctly" in {
    val encryptedAddress = SensitiveRequestReferenceNumberAddress(clearAddress)
    encryptedAddress.decryptedValue shouldBe clearAddress
  }

  it should "serialize to encrypted JSON" in {
    val encryptedValue = SensitiveRequestReferenceNumberAddress(clearAddress)
  ...
  it should "deserialize from encrypted JSON" in {
    ...
    val deserialized     = Json.fromJson[SensitiveRequestReferenceNumberAddress](jsValue)
    deserialized shouldBe JsSuccess(encryptedAddress)
  }

  val clearAddress = RequestReferenceNumberAddress(
