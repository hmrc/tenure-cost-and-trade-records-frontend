...
class SensitiveEditAddressSpec extends AnyFlatSpec with Matchers with OptionValues with MongoCryptoSupport:

  it should "encrypt and decrypt sensitive fields correctly" in {
    val encryptedAddress = SensitiveEditAddress(clearAddress)
    encryptedAddress.decryptedValue shouldBe clearAddress
  }

  it should "serialize to encrypted JSON" in {
    val encryptedValue = SensitiveEditAddress(clearAddress)
  ...
  it should "deserialize from encrypted JSON" in {
    ...
    val deserialized     = Json.fromJson[SensitiveEditAddress](jsValue)
    deserialized shouldBe JsSuccess(encryptedAddress)
  }

  val clearAddress = EditAddress(
