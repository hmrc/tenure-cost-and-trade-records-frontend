...
class PitchesForCaravansController @Inject() (
  ...
  def show: Action[AnyContent] = (Action andThen withSessionRefiner).async { implicit request =>
    runWithSessionCheck { turnoverSections6045 =>
      ...
      Ok(
        view(
          pitchesForCaravansForm(years).fill(
            turnoverSections6045.map(_.pitchesForCaravans getOrElse PitchesForCaravans())
          ),
          getBackLink
        )
      )
  ...
  def submit: Action[AnyContent] = (Action andThen withSessionRefiner).async { implicit request =>
    runWithSessionCheck { turnoverSections6045 =>
      ...
      continueOrSaveAsDraft[Seq[PitchesForCaravans]](
        pitchesForCaravansForm(years),
