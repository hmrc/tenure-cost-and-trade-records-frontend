...
class PremisesLicenseGrantedController @Inject()(
  ...
  def show: Action[AnyContent] = Action { implicit request =>
    Ok(premisesLicenseGrantedView(premisesLicenseGrantedForm))
  }
  ...
  def submit = (Action andThen withSessionRefiner).async { implicit request =>
    premisesLicenseGrantedForm
      ...
      .fold(
        ...
        data =>
          Future.successful(Redirect(navigator.nextPage(PremisesLicenseGrantedId).apply(request.sessionData)))
      )
  }
}
