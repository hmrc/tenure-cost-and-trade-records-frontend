...
class HasStoppedLettingController @Inject (
  ...
  def submit: Action[AnyContent] = (Action andThen sessionRefiner).async { implicit request =>
    continueOrSaveAsDraft[AnswersYesNo](
      ...
      answer =>
        ...
        for
          savedSession  <- saveOrUpdateWithHasStopped(Some(answer.toBoolean), request.sessionData)
  ...
  private def saveOrUpdateWithHasStopped(hasStopped: Option[Boolean], session: Session)(using
    ws: Writes[Session],
    hc: HeaderCarrier,
    ec: ExecutionContext
  ): Future[Session] =
    given Session = session
    repository.saveOrUpdateSession(withHasStopped(hasStopped))
