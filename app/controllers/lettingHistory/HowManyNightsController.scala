  ...
  def submit: Action[AnyContent] = (Action andThen sessionRefiner).async { implicit request =>
    continueOrSaveAsDraft[Int](
      ...
      nights =>
        ...
        for
          savedSession  <- repository.saveOrUpdateSession(withNumberOfNights(nights))
          meetsCriteria <- doesMeetLettingCriteria(savedSession).toFuture
          navigationData = Map("meetsCriteria" -> meetsCriteria.toString)
        yield navigator.redirect(currentPage = HowManyNightsPageId, savedSession, navigationData)
    )
  }
