...
class FileUploadController @Inject() (mcc: MessagesControllerComponents,
                                      ...
                                      fileUploadView:fileUpload)(implicit ec:ExecutionContext) extends FrontendController(mcc) with I18nSupport {
  ...
  def AuthenticatedAction[A](parser: BodyParser[A])(f: Request[A] => Result): Action[A] = Action(parser) { implicit request =>
  ...
  def AuthenticatedActionAsync[A](parser: BodyParser[A])(f: Request[A] => Future[Result]): Action[A] = Action.async(parser) { implicit request =>
    request.session.get("authenticated") match {
      case Some(_) => f(request)
