...
import views.html.feedback.feedbackThx
...
class SurveyController @Inject() (
  ...
  def formCompleteFeedback = refNumAction.async { implicit request =>
    completedFeedbackForm.bindFromRequest().fold(
      formWithErrors => Future.successful(BadRequest(feedbackView(formWithErrors))),
      success => {
        sendFeedback(success, request.refNum) map { _ => Redirect(routes.FeedbackController.feedbackThx) }
      }
    )
  }
