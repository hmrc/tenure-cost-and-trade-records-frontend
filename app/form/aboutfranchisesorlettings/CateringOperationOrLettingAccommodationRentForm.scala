...
object CateringOperationOrLettingAccommodationRentForm {
  ...
  def cateringOperationOrLettingAccommodationRentForm(using messages: Messages): Form[CateringOperationRentDetails] =
    Form(
      mapping(
        "annualRent" -> currencyMapping(".annualRent"),
        "dateInput"  -> requiredDateMapping("cateringOrLettingFixedRentDate", allowPastDates = true)
      )(CateringOperationRentDetails.apply)(o => Some(Tuple.fromProductTyped(o)))
    )
