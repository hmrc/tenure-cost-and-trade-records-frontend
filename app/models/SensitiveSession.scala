...
import models.submissions.aboutYourLeaseOrTenure.{AboutLeaseOrAgreementPartTwo, SensitiveAboutLeaseOrAgreementPartOne}
...
case class SensitiveSession (referenceNumber: String,
                             ...
                             requestReferenceNumber: Option[SensitiveRequestReferenceNumber]) extends Sensitive[Session]{
  ...
  override def decryptedValue:Session = Session(
    ...
    saveAsDraftPassword,
    requestReferenceNumber.map(_.decryptedValue)
  )
}
...
object SensitiveSession {
  ...
  def apply(session: Session):SensitiveSession = SensitiveSession(
    ...
    session.requestReferenceNumber.map(SensitiveRequestReferenceNumber(_))
  )
}