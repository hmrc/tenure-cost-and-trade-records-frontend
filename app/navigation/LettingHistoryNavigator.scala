...
class LettingHistoryNavigator @Inject() (audit: Audit) extends Navigator(audit) with RentalPeriodSupport:
  ...
  private val forwardNavigationMap = Map[Identifier, NavigationFunction](
    ...
    HowManyNightsPageId      -> { (currentSession, navigationData) =>
      // assert navigationData.isDefinedAt("meetsCriteria"))
      for meetsCriteria <- navigationData.get("meetsCriteria").map(_.toBoolean)
      yield
        if meetsCriteria
